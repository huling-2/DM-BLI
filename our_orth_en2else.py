...
def evaluate(args, emb1, emb2, src_word2ind, src_ind2word, trg_ind2word):
    ...
    file_lexicon = open(
        f'/student15/hl/clwe/dictionaries/{args.src_lang}-{args.trg_lang}.5000-6500.txt', 'r',
        encoding='utf-8')
    src_gold_words = []
    trg_gold_words = []
    binary_lexicon = {}
...
def left_fill(args, all_pairs, new_trg_emb_words_alreay, trg_word2ind, trg_ind2word, src_ind2word, n_clusters,subspace_dico_new,z_tensor,mapping_wz,trgfile):
    if len(new_trg_emb_words_alreay) != 75000:
        id_t = [trg_word2ind[t_word] for t_word in new_trg_emb_words_alreay]
        not_in_list = list(set([k for k in range(75000)]) - set(id_t))
        print(len(not_in_list))

        t_sub_left = {}
        for pair in all_pairs:
            if int(pair[0]) in not_in_list:
                sword = src_ind2word[float(pair[1])]
                tword = trg_ind2word[float(pair[0])]
                num = subspace_dico_new[sword]
                if num not in t_sub_left:
                    t_sub_left.update({num: [tword]})
                else:
                    t_sub_left[num].append(tword)

        for j in range(n_clusters):
            if j in t_sub_left.keys():
                left_id_t = [trg_word2ind[t_word] for t_word in t_sub_left[j]]
                print(f"#####{j}:{len(left_id_t)}")
                left_trg_emb_select = torch.index_select(z_tensor, 0, torch.tensor(left_id_t).cuda())
                to_reload_wz = cp.asnumpy(torch.load(os.path.join(args.sub_mappingpath, f'best_mapping_wz_{j}.pth')))
                to_reload_wz = torch.from_numpy(to_reload_wz).cuda()
                mapping_wz.weight.data.copy_(to_reload_wz.type_as(mapping_wz.weight.data))
                left_trg_emb_mapped = mapping_wz(left_trg_emb_select)
                embeddings.write(t_sub_left[j], cp.asarray(left_trg_emb_mapped.detach().cpu().numpy()), trgfile)
                del left_trg_emb_select, left_trg_emb_mapped
...
def main():
    ...
    parser.add_argument('--trg_lang', default='bg', help='target language')
    ...
    parser.add_argument('--mappingpath', type=str, default='/student15/hl/clwe/mapping', help='path to save the optimal mapping path')
    parser.add_argument('--sub_mappingpath', type=str, default='/student15/hl/clwe/sub_best_mapping',
                        help='path to save the optimal mapping path')
    parser.add_argument('--hyper', type=float, default=0.48)
    ...
    df = pd.read_csv(f'cluster_output_kmeans-{n_clusters}-75000.csv', keep_default_na=False)
    ...
    for index, row in df.iterrows():
        num = row['Cluster']
        if num not in s_sub:
            s_sub.update({num: [row['words']]})
        else:
            s_sub[num].append(row['words'])
    ...
    # x_map --> en; z_map --> zh
    all_pairs = get_candidates(z_mapped, x_mapped)
    ...
    for i in s_sub_by_length_desc.keys():
        ...
        while stablity < 0.95:
            ...
            while count <3 :
                ...
                id_alr = [trg_word2ind[t_word] for t_word in new_trg_emb_words_alreay]
                id_t = list(set(id_t)- (set(id_t)&set(id_alr)))
                ...
                select_pairs = get_candidates(trg_emb_mapped, src_emb_mapped)
                v_limit = int(0.20 * len(select_pairs))
                select_pairs = select_pairs[:v_limit]
                # print(select_pairs)
                cl_important_src_emb_mapped = torch.index_select(src_emb_mapped, 0, torch.tensor(select_pairs[:,1]).cuda())
                cl_important_trg_emb_mapped_pos = torch.index_select(trg_emb_mapped, 0, torch.tensor(select_pairs[:,0]).cuda())
            ...
            new_pairs = get_candidates(new_z_mapped, new_x_mapped)

            new_trg_words = []
            for pair in new_pairs:
                tword = trg_ind2word[float(pair[0])]
                if tword not in new_trg_emb_words_alreay:
                    sword = src_ind2word[float(pair[1])]
                    num = subspace_dico_new[sword]
                    if num == i:
                        new_trg_words.append(tword)


            common_elements = set(t_sub[i]) & set(new_trg_words)
            stablity = len(common_elements) / len(set(t_sub[i]))
            print(stablity)
            t_sub[i] = new_trg_words
            iter += 1

        already.append(i)
        new_trg_emb_words_alreay += t_sub[i]
        print(f"###{i}: {len(t_sub[i])}")
        print("*"*30, "len(new_trg_emb_words_alreay):", len(new_trg_emb_words_alreay))
        print("*"*30, "set-len(new_trg_emb_words_alreay):", len(set(new_trg_emb_words_alreay)))

        evaluate(args, new_x_mapped.detach().cpu().numpy(), new_z_mapped.detach().cpu().numpy(), src_word2ind, src_ind2word, trg_ind2word)

        new_id_t = [trg_word2ind[t_word] for t_word in t_sub[i]]
        new_trg_emb_select = torch.index_select(z_tensor, 0, torch.tensor(new_id_t).cuda())
        new_trg_emb_mapped = mapping_wz(new_trg_emb_select)
        new_src_emb_mapped = mapping_wx(src_emb_select)
        new_z_mapped[new_id_t, :] = new_trg_emb_mapped
        new_x_mapped[id_s, :] = new_src_emb_mapped
        ...
        del new_trg_emb_select, new_src_emb_mapped, new_trg_emb_mapped, new_id_t, w_x_update, w_z_update, src_emb_select
        torch.cuda.empty_cache()

    print(len(new_trg_emb_words_alreay))
    print("*" * 30, "len(new_trg_emb_words_alreay):", len(new_trg_emb_words_alreay))
    print("*" * 30, "set-len(new_trg_emb_words_alreay):", len(set(new_trg_emb_words_alreay)))
    left_fill(args, all_pairs, new_trg_emb_words_alreay, trg_word2ind, trg_ind2word, src_ind2word, n_clusters, subspace_dico_new,z_tensor,mapping_wz, trgfile)
